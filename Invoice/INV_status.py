...
currentDate = date.today() - timedelta(days=200)
...
invCount = requests.get(data["sapurl"]+"/Invoices/$count?$filter=PaidToDateSys gt 0 and UpdateDate ge "+str(currentDate), cookies=r.cookies, verify=False).text
...
if int(count) > 0:
    ...
    for i in range(count):
        # baseUrl = data["sapurl"]+"/Invoices?$orderby = DocEntry desc&$select=DocEntry,DocNum,DocType,CardCode,CardName,DocumentStatus,UpdateDate,CancelStatus,PaidToDateSys,DocDate,DocDueDate,TaxDate,CreationDate&$skip="+str(skip)
        baseUrl = data["sapurl"]+"/Invoices?$orderby = DocEntry asc&$select=DocEntry,DocNum,DocType,CardCode,CardName,DocumentStatus,UpdateDate,CancelStatus,PaidToDateSys,DocDate,DocDueDate,TaxDate,CreationDate&$filter=PaidToDateSys gt 0 and UpdateDate ge '"+str(currentDate)+"'&$skip="+str(skip)
        print(baseUrl)
