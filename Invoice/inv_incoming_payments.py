...
# res = requests.get(data['sapurl']+'/IncomingPayments?$filter=DocDate eq '+str(currentDate)+'', headers={'Authorization': "Bearer "+data['SessionId']+""}, verify=False)
# res = requests.get(data['sapurl']+'/IncomingPayments?$filter=DocDate gt '+str(currentDate), cookies=r.cookies, verify=False)
