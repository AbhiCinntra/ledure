...
if True:
    ...
    while skip != "": 
        ...
        sapAPIUrl = f"/Invoices?$filter = DocEntry gt {lastDocEntry}&$skip = {skip}&$orderby=DocEntry"
        # sapAPIUrl = f"/Invoices?$orderby = DocEntry asc&$filter = DocEntry ge {lastDocEntry}&$skip = {skip}"
        print("sapAPIUrl", sapAPIUrl)
        ...
        for opt in opts['value']:
            ...
            if mycursor.rowcount != 1:
                ...
                # >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
                seriesNameURL = f"http://65.2.148.88:8000/Ledure/General/SeriesName.xsjs?DBName=LEDURE_LIVE_300323&DocEntry={DocEntry}&ObjType=13"
                print(seriesNameURL)
                ...
                for line in opt['DocumentLines']:
                    ...
                    #>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
                    if HSNEntry != "None":
                        hsnAPIRsponse = requests.get(f"http://65.2.148.88:8000/Ledure/General/GetHSN.xsjs?DBName=LEDURE_LIVE_300323&AbsEntry={HSNEntry}", cookies=r.cookies, verify=False)
                        ...
                        sacAPIRsponse = requests.get(f"http://65.2.148.88:8000/Ledure/General/GetSacName.xsjs?DBName=LEDURE_LIVE_300323&AbsEntry={SACEntry}", cookies=r.cookies, verify=False)
