        ...
        for groupObj in categoryObjs:
            ...
            sqlQuery = f"""
                ...
                WHERE 
                    ...
                    {fromToDate}
                GROUP BY Invoice_documentlines.`U_UTL_ITSBG` {orderby} LIMIT 20
            """
                    # AND Item_item.ItemsGroupCode = '{groupObj.Number}'
...
def sold_items_dashboard(request):
    try:
        ...
        print('^^^^^^^^^^^^^^^^^')
        # sqlQuery2 = f"SELECT Invoice_documentlines.`id`, Invoice_documentlines.`ItemDescription` as ItemName, Invoice_documentlines.ItemCode as ItemCode, SUM(Invoice_documentlines.LineTotal ) AS TotalPrice, SUM(Invoice_documentlines.Quantity) AS TotalQty, (LineTotal/Quantity) as UnitPrice, count(DISTINCT Invoice_documentlines.InvoiceID) as NoOfInvoice FROM `Invoice_documentlines` INNER JOIN Item_item ON Item_item.ItemCode = Invoice_documentlines.ItemCode INNER JOIN Invoice_invoice ON Invoice_invoice.id = Invoice_documentlines.InvoiceID INNER Join BusinessPartner_businesspartner bp ON bp.CardCode = Invoice_invoice.CardCode WHERE bp.U_U_UTL_Zone IN('{zonesStr}') AND Invoice_invoice.CancelStatus = 'csNo' {fromToDate};"
        sqlQuery2 = f"SELECT Invoice_documentlines.`id`, Invoice_documentlines.`ItemDescription` as ItemName, Invoice_documentlines.ItemCode as ItemCode, SUM(Invoice_documentlines.LineTotal ) AS TotalPrice, SUM(Invoice_documentlines.Quantity) AS TotalQty, (LineTotal/Quantity) as UnitPrice, count(DISTINCT Invoice_documentlines.InvoiceID) as NoOfInvoice FROM `Invoice_documentlines` INNER JOIN Item_item ON Item_item.ItemCode = Invoice_documentlines.ItemCode INNER JOIN Invoice_invoice ON Invoice_invoice.id = Invoice_documentlines.InvoiceID WHERE Invoice_invoice.CancelStatus = 'csNo' {fromToDate};"
        print(sqlQuery2)
        ...
        #         endWith = (PageNo * size)
        #         startWith = (endWith - size)
        #         itemList = itemList[startWith:endWith]
