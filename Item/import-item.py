...
lastCreatedDate = str(currentDate)
mycursor.execute("SELECT DISTINCT CreatedDate FROM `Item_item` ORDER BY CreatedDate DESC limit 1")
entryData = mycursor.fetchall()
if len(entryData) > 0:
    lastCreatedDate = entryData[0]['CreatedDate']

print("lastCreatedDate", lastCreatedDate)
...
SAPURL_Count = f"{settings.SAPURL}/Items/$count?$filter = CreateDate ge '{lastCreatedDate}'"
print("SAPURL_Count", SAPURL_Count)
res = requests.get(SAPURL_Count, cookies=ses.cookies, verify=False)
# print(res.text)
...
for page in range(pages):
    ...
    # res = requests.get(settings.SAPURL+'/Items?$skip='+str(skip), cookies=ses.cookies, headers={"Prefer":"odata.maxpagesize=100"}, verify=False)
    SAPURL_Get = f"{settings.SAPURL}/Items?$filter = CreateDate ge '{lastCreatedDate}'&$skip={str(skip)}"
    print("SAPURL_Get", SAPURL_Get)
    res = requests.get(SAPURL_Get, cookies=ses.cookies, verify=False)
