...
for page in range(pages):
    ...
    for item in items['value']:
        ...
        if rc >= 1:
            ...
            U_UTL_ITSBG = item['U_UTL_ITSBG']
            U_UTL_ITMCT = item['U_UTL_ITMCT']
            # >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
            ...
            # >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
            # >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
            item_sql = f'UPDATE `Item_item` SET `ItemName`="{ItemName}",`Inventory`="{Inventory}", `Status`="{Status}", `CreatedDate` = "{CreatedDate}", `CreatedTime`="{CreatedTime}", `UpdatedDate`="{UpdatedDate}",`UpdatedTime`="{UpdatedTime}",`UoS`="{UoS}", `UnitWeight`="{UnitWeight}", `GSTTaxCategory` = "{GSTTaxCategory}", `SalesItemsPerUnit` = "{SalesItemsPerUnit}", `UoMIds` = "{UoMIds}", `U_UTL_ITSBG` = "{U_UTL_ITSBG}", `U_UTL_ITMCT` = "{U_UTL_ITMCT}" WHERE `ItemCode` = "{ItemCode}"'
            print(item_sql)
