...
invCount = requests.get(data["sapurl"]+"/VendorPayments/$count", cookies=r.cookies, verify=False).text
...
if int(count) > 0:
    ...
    for i in range(count):
        baseUrl = data["sapurl"]+"/VendorPayments?$orderby = DocEntry asc&$select=DocEntry,DocNum,DocType,CardCode,CardName,JournalRemarks&$skip="+str(skip)
        print(baseUrl)
