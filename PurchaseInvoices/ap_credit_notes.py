...
if True:
    ...
    while skip != "": 
        # res = requests.get(data['sapurl']+'/PurchaseCreditNotes?$filter=UpdateDate ge '+str(currentDate)+'&$skip='+str(skip)+'', cookies=r.cookies, verify=False)
        #sapAPIUrl = f"/PurchaseCreditNotes?$filter = DocEntry ge {lastDocEntry}&$skip = {skip}&$orderby=DocEntry"
        ...
        for opt in opts['value']:
            # try:
                ...
                if mycursor.rowcount != 1:
                    ...
                    # >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
                    seriesNameURL = f"http://35.154.67.167:8000//Ledure/General/SeriesName.xsjs?DBName=LEDURE_LIVE_300323&DocEntry={DocEntry}&ObjType=19"
                    print(seriesNameURL)
