...
invCount = requests.get(data["sapurl"]+"/PurchaseCreditNotes/$count", cookies=r.cookies, verify=False).text
...
if int(count) > 0:
    ...
    for i in range(count):
        baseUrl = data["sapurl"]+"/PurchaseCreditNotes?$orderby = DocEntry asc&$select=DocEntry,DocNum,DocType,CardCode,CardName,DocumentStatus,UpdateDate,CancelStatus&$skip="+str(skip)
        print(baseUrl)
