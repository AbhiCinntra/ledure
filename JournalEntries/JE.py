...
def log_error(message):
    ...
    filename = "/home/www/b2b/ledure_dev/bridge/LogFiles/je_cronjob_error_log.txt"
        ...
        for opt in opts['value']:
            ...
            if mycursor.rowcount == 0:
                ...
                if mycursor.rowcount > 0:
                    CNNo = JdtNum
                    # >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
                    seriesNameURL = f"http://35.154.67.167:8000/Ledure/General/SeriesName.xsjs?DBName=LEDURE_LIVE_300323&DocEntry={JdtNum}&ObjType=30"
                    # print(seriesNameURL)
...
# >>>>>>>>>>>>>>>>>>>>
    ...
    error_message = f"Exception From Journal Entries: {str(e)}"
