...
if int(count) > 0:
    ...
    for i in range(count):
        res = requests.get(data["sapurl"]+"/Orders?$select=DocEntry,DocNum,DocType,CardCode,CardName,DocumentStatus,UpdateDate,CancelStatus&$filter=DocumentStatus ne 'bost_Open'and UpdateDate ge "+str(currentDate)+"&$skip="+str(skip), cookies=r.cookies, verify=False)
