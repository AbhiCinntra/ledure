...
#     # password='root',
#     database='shivtara_pre'
# )
# mycursor = mydb.cursor(buffered=True)

# file_path = os.path.realpath(__file__)
file_path = str("/home/www/b2b/ledure_pre/bridge/BusinessPartner/")
print(">>>>>>>>>>>>>>>>>>>>>>>>>")
...
# print("<><><><><><><><><><><>><><><><><><")
# print("===== Login SAP ====")
# data = {"CompanyDB": "SHIVTARA_LIVE_19_01_2023", "Password": "Shiv@1234", "UserName": "manager", "SessionId": "8e2a8660-ad05-11ed-8000-005056a40bab", "at": "2023-02-15 13:21:28", "sapurl": "http://14.97.43.205:50001/b1s/v1"}
# r = requests.post(data['sapurl']+'/Login', data=json.dumps(data), verify=False)
# print(r)
# currentDate = '2023-02-10'

# serBPCount = requests.get(settings.SAPURL+"/BusinessPartners/$count?$filter = GroupCode eq 112 and Valid eq 'tYES'", cookies=ses.cookies, verify=False).text
# serBPCount = requests.get(settings.SAPURL+"/BusinessPartners/$count?$filter = CardType eq 'cCustomer' and Valid eq 'tYES'", cookies=ses.cookies, verify=False).text
serBPCount = requests.get(settings.SAPURL+"/BusinessPartners/$count?$filter = CardCode eq '8255'", cookies=ses.cookies, verify=False).text
...
if True:
    ...
    for i in range(count):
        ...
        bpRes = requests.get(settings.SAPURL+"/BusinessPartners?$filter = CardCode eq '8255'&$skip="+str(skip), cookies=ses.cookies, verify=False)
